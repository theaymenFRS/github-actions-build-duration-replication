...
LOG_FILE = "xgboost_placeholder.log"
...
def generate_individual():
    params = {
        ...
        "learning_rate": 10 ** random.uniform(-2.0, -1.1),  # ~0.01–0.08
        "max_depth": random.randint(3, 12),
        "min_child_weight": 10 ** random.uniform(-0.3, 1.2),  # ~0.5–16
        "subsample": random.uniform(0.6, 0.95),
        "colsample_bytree": random.uniform(0.6, 1.0),
        "reg_lambda": 10 ** random.uniform(-0.3, 1.5),  # ~0.5–30
        "reg_alpha": 10 ** random.uniform(-2.0, 0.7),  # ~0.01–5
        "gamma": 10 ** random.uniform(-2.0, 0.7),  # ~0.01–5 (optional but useful)
    }
...
def mutate(individual):
    ...
    if k == "n_estimators":
        p[k] = int(max(200, min(3000, p[k] + random.randint(-200, 200))))
    elif k == "learning_rate":
        p[k] = max(0.005, min(0.2, p[k] * (10 ** random.uniform(-0.2, 0.2))))
    elif k == "max_depth":
        p[k] = int(max(3, min(16, p[k] + random.choice([-2, -1, 1, 2]))))
    elif k == "gamma":
        p[k] = max(0.0, min(20.0, p[k] * (10 ** random.uniform(-0.3, 0.3))))
    elif k == "min_child_weight":
        p[k] = max(0.1, min(50.0, p[k] * (10 ** random.uniform(-0.3, 0.3))))
    elif k in ("subsample", "colsample_bytree"):
        p[k] = max(0.4, min(1.0, p[k] + random.uniform(-0.1, 0.1)))
    elif k == "reg_lambda":
        p[k] = max(0.1, min(50.0, p[k] * (10 ** random.uniform(-0.3, 0.3))))
    elif k == "reg_alpha":
        p[k] = max(0.0, min(10.0, p[k] * (10 ** random.uniform(-0.3, 0.3))))
...
if __name__ == "__main__":
    ...
    out_dir = REPO_ROOT / "results" / "rq1" / "xgboost"
    ...
    for project_name, file_path in PROJECTS:
        ...
        LOG_FILE = str(out_dir / f"xgboost_{project_name}_RQ1.log")
