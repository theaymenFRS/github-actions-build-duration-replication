...
LOG_FILE = "lightgbm_placeholder.log"
...
def fitness_ga_iter(individual, X, y, kept_features, gen_num, ind_num, k_iter):
    ...
    for wf_id in X["workflow_id"].unique():
        ...
        pred = np.expm1(model.predict(X_va.to_numpy()))
...
def generate_individual():
    # LightGBM hyperparameter ranges (GA will search inside these)
    params = {
        "n_estimators": int(10 ** random.uniform(2.6, 3.3)),   # ~400–2000
        "learning_rate": 10 ** random.uniform(-2.2, -1.0),     # ~0.006–0.10
        "num_leaves": int(2 ** random.uniform(4, 8)),          # ~16–256
        "max_depth": random.randint(-1, 16),                   # -1 means no limit
        "min_child_samples": int(10 ** random.uniform(1.0, 2.2)),  # ~10–160
        "subsample": random.uniform(0.6, 1.0),                 # bagging_fraction
        "colsample_bytree": random.uniform(0.6, 1.0),          # feature_fraction
        "reg_lambda": 10 ** random.uniform(-1.0, 1.7),         # ~0.1–50
        "reg_alpha": 10 ** random.uniform(-2.0, 1.0),          # ~0.01–10
    }
...
def mutate(individual):
    ...
    if k == "n_estimators":
        p[k] = int(max(200, min(4000, p[k] + random.randint(-200, 200))))

    elif k == "learning_rate":
        p[k] = max(0.003, min(0.2, p[k] * (10 ** random.uniform(-0.25, 0.25))))

    elif k == "num_leaves":
        p[k] = int(max(8, min(512, p[k] + random.randint(-16, 16))))

    elif k == "max_depth":
        # keep -1 allowed, otherwise clamp 1..32
        if p[k] == -1 and random.random() < 0.5:
            p[k] = random.randint(3, 16)
        else:
            p[k] = random.choice([-1] + list(range(3, 17)))

    elif k == "min_child_samples":
        p[k] = int(max(5, min(500, p[k] + random.randint(-10, 10))))

    elif k in ("subsample", "colsample_bytree"):
        p[k] = max(0.4, min(1.0, p[k] + random.uniform(-0.1, 0.1)))

    elif k == "reg_lambda":
        p[k] = max(0.0, min(100.0, p[k] * (10 ** random.uniform(-0.3, 0.3))))

    elif k == "reg_alpha":
        p[k] = max(0.0, min(50.0, p[k] * (10 ** random.uniform(-0.3, 0.3))))
...
if __name__ == "__main__":
    ...
    out_dir = REPO_ROOT / "results" / "rq1" / "lightgbm"
    ...
    for project_name, file_path in PROJECTS:
        ...
        LOG_FILE = str(out_dir / f"lightgbm_{project_name}_RQ1.log")
        ...
        # --- Baseline for this project (same folds as model evaluation) ---
