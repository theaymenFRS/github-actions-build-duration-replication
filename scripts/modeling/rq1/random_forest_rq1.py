...
LOG_FILE = "random_forest_placeholder.log"
...
def fitness_ga_iter(individual, X, y, kept_features, gen_num, ind_num, k_iter):
    ...
    for wf_id in X["workflow_id"].unique():
        ...
        # IMPORTANT: do NOT log-transform targets for RF (optional, but I'd start without it)
...
def generate_individual():
    params = {
        "n_estimators": random.randint(200, 1500),
        "max_depth": random.choice([None] + list(range(4, 41))),  # None allowed
        "min_samples_split": random.randint(2, 20),
        "min_samples_leaf": random.randint(1, 10),
        "max_features": random.choice(["sqrt", "log2", None]),  # None = all features
...
def mutate(individual):
    ...
    if k == "n_estimators":
        p[k] = int(max(100, min(3000, p[k] + random.randint(-200, 200))))
        ...
        p[k] = random.choice(["sqrt", "log2", None])
...
if __name__ == "__main__":
    ...
    out_dir = REPO_ROOT / "results" / "rq1" / "random_forest"
    ...
    for project_name, file_path in PROJECTS:
        ...
        LOG_FILE = str(out_dir / f"random_forest_{project_name}_RQ1.log")
