...
import matplotlib
# from sklearn.ensemble import RandomForestRegressor   # <-- removed / not used now
...
LOG_FILE = "decision_tree_placeholder.log"
...
def fitness_ga_iter(individual, X, y, kept_features, gen_num, ind_num, k_iter):
    ...
    for wf_id in X["workflow_id"].unique():
        ...
        model = DecisionTreeRegressor(   # <-- CHANGED
            **params,
            random_state=42
        )
...
def generate_individual():
    # DecisionTreeRegressor hyperparameter search space
    params = {
        "max_depth": random.choice([None] + list(range(2, 61))),
        ...
        "criterion": random.choice(["squared_error", "friedman_mse", "absolute_error"]),
    }
...
def mutate(individual):
    ...
    if k == "max_depth":
        if random.random() < 0.3:
            ...
            cur = 10 if p[k] is None else p[k]
            p[k] = int(max(2, min(60, cur + random.choice([-5, -3, -1, 1, 3, 5]))))

    elif k == "min_samples_split":
        p[k] = int(max(2, min(50, p[k] + random.choice([-2, -1, 1, 2]))))

    elif k == "min_samples_leaf":
        p[k] = int(max(1, min(30, p[k] + random.choice([-2, -1, 1, 2]))))

    elif k == "max_features":
        p[k] = random.choice([None, "sqrt", "log2"])

    elif k == "splitter":
        p[k] = random.choice(["best", "random"])

    elif k == "criterion":
        p[k] = random.choice(["squared_error", "friedman_mse", "absolute_error"])
...
PROJECTS = [
    ("daos", RAW_DATA_DIR / "daos_wf9020028_fixed.csv"),
    ("rustlang", RAW_DATA_DIR / "rustlang_wf51073_fixed.csv"),
    ("orange", RAW_DATA_DIR / "Orange_OpenSourceouds_android_wf108176393_fixed.csv"),
    ("bmad", RAW_DATA_DIR / "bmad simbmad ecosystem_wf69576399_fixed.csv"),
    ("ccpay", RAW_DATA_DIR / "ccpay_wf6192976_fixed.csv"),
    ("filterlists", RAW_DATA_DIR / "collinbarrettFilterLists_wf75763098_fixed.csv"),
    ("jod", RAW_DATA_DIR / "jod-yksilo-ui_wf83806327_fixed.csv"),
    ("m2os", RAW_DATA_DIR / "m2Gilesm2os_wf105026558_fixed.csv"),
    ("bruce", RAW_DATA_DIR / "pr3y_Bruce_wf121541665_fixed.csv"),
    ("radare2", RAW_DATA_DIR / "radareorg_radare2_wf1989843_fixed.csv"),
]

if __name__ == "__main__":
    plt.ion()  # interactive on if you still want live plot windows

    out_dir = REPO_ROOT / "results" / "rq1" / "decision_tree"
    ...
    for project_name, file_path in PROJECTS:
        ...
        LOG_FILE = str(out_dir / f"decision_tree_{project_name}_RQ1.log")

        if os.path.exists(LOG_FILE):
            os.remove(LOG_FILE)

        # --- Preprocess data for this project ---
        processed_out = PROCESSED_DATA_DIR / f"{project_name}_processed_2025.csv"
        ...
        # --- GA for this project (Decision Tree) ---
